...
Butuh: Windows 10/11, Python 3.9+, PyQt6, psutil.  Jalankan sebagai Administrator.
"""
...
class RamWorker(QThread):
    finished_ok = pyqtSignal(float, list, list)   # freed_bytes, done[], failed[]
...
class CpuWorker(QThread):
    ...
    finished_ok = pyqtSignal(int, int)   # apps, processes
...
class SpeedTestWorker(QThread):
    ...
    result = pyqtSignal(float, float, float)   # ping ms, down Mbps, up Mbps
